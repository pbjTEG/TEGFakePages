...
class TEGFakePages {
	...
	constructor(Options) {
		...
		__instance.options = {
			...
			pageChange: {
				'0': {
					beforeShow: function (pageNumber, pageObject, fakePages) {
						// clear errors before showing a new page
						pageObject.find(fakePages.options.errorSelector).hide();
						return true;
					}, // end beforeShow()
					beforeHide: function (pageNumber, pageObject, fakePages) {
						// Are we validating?
						if (fakePages.options.runValidation === 'none') {
							// If not, just go.
							return true;

						} else {
							// Otherwise, don't leave if errors are visible.
							return pageObject.find(fakePages.options.errorSelector).is(':visible');
						}
					}, // end beforeHide()
		...
		jQuery.extend(true, __instance.options, Options);
		...
		if (__instance.options.pageLength > 0) {
			...
			__instance.#lastPageNumber = jQuery(__instance.options.pageStartSelector)
				.each(function (index) {
					...
					if (__instance.options.buttonPlacement > TEGFakePages.BUTTON_NONE &&
						 __instance.options.pageButtons.length > 0) {
						const newButtons  = __instance.options
																.pageButtons
																.clone(),
	...
	goPage(page, goPageRunValidation) {
		...
		if (this.options.pageChange['0'].hasOwnProperty('beforeHide')) {
			this.options.pageChange['0'].beforeHide(this.#currentPageNumber, this.#currentPageObject, this);
		...
		if (this.options.pageChange.hasOwnProperty(this.#currentPageNumber) &&
			...
			if (!this
				...
				.beforeHide(this.#currentPageNumber, this.#currentPageObject, this)) {
				// do not proceed
				return false;
		...
		if (this.options.pageChange?.['0']?.hasOwnProperty('beforeShow')) {
			this.options.pageChange['0'].beforeShow(page, this.#pageObjects.eq(page), this);
		...
		if (this.options.pageChange.hasOwnProperty(page) &&
			...
			if (!this
				.options
				.pageChange[page]
				.beforeShow(page, this.body.find('section#' + this.options.pageIDPrefix + page), this))
			{
	...
	errorPage() {
		...
		for (let counter = 0; counter < this.#pageObjects.length; counter++) {
